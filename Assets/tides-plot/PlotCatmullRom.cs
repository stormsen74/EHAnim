...
public class PlotCatmullRom {
    ...
    [System.Serializable]
    public struct CatmullRomPointPosition {
        public Vector3 position;

        public CatmullRomPointPosition(Vector3 position) {
            this.position = position;
        }
    }
    ...
    public PlotCatmullRom(Vector3[] controlPoints, int resolution) {
    ...
    //Evaluates curve at t[0, 1]. Returns point/normal/tan struct. [0, 1] means clamped between 0 and 1.
    public static void EvaluatePosition(Vector3 start, Vector3 end, Vector3 tanPoint1, Vector3 tanPoint2, float t, out CatmullRomPointPosition result) {
        result.position = CalculatePosition(start, end, tanPoint1, tanPoint2, t);
    }
